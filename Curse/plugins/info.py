...
async def user_info(c: app, user, already=False):
    ...
    if not user.first_name:
        return ["⚰️ This ghost has no name.", None]
    ...
    if user_id == OWNER_ID:
        ...
        role = "🧙‍♂️ Enchanted Assistant (Bot)"
    elif user_id in DEV_USERS:
        role = "🔧 Order of Developers"
    elif user_id in SUDO_USERS:
        role = "🛡️ Hogwarts Council (Sudo)"
    ...
    gban_status = "☠️ Cursed (GBanned)" if gban_status else "✅ No Dark Magic Detected"
...
🧙‍♂️ <b>Username:</b> @{user.username or "None"}
...
🕵️‍♂️ <b>Scam:</b> {"✅" if user.is_scam else "❌"}
...
async def info_func(c: app, message: Message):
    ...
    if not user:
        return await message.reply_text("🧙‍♂️ You need to specify a target for the reveal spell!")
    ...
    try:
        ...
        return await m.edit("⚠️ The spell backfired!\n\n" + str(e))
...
__HELP__ = """
🧙‍♂️ <b><u>Wizard Identity Spells</u></b>
