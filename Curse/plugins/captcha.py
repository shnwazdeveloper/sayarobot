...
from Curse import LOGGER, MESSAGE_DUMP
from Curse.bot_class import app
from Curse.database.captcha_db import CAPTCHA, CAPTCHA_DATA
from Curse.supports import get_support_staff
from Curse.utils.captcha_helper import (genrator, get_image_captcha,
                                         get_qr_captcha)
from Curse.utils.custom_filters import admin_filter, captcha_filter, command
from Curse.utils.extras import BAN_GIFS


@app.on_message(command("captcha") & admin_filter & ~filters.private)
...
@app.on_message(command("captchamode") & admin_filter & ~filters.private)
async def set_captcha_mode(c: app, m: Message):
    split = m.command
    captcha = CAPTCHA()
    if len(split) == 1:
...
@app.on_callback_query(filters.regex("^captcha_"))
async def captcha_codes_check(c: app, q: CallbackQuery):
...
@app.on_message(filters.group & captcha_filter & filters.new_chat_members, group=3)
async def on_chat_members_updatess(c: app, m: Message):
    chat = m.chat.id
